...
define('DIV_PHP_ALLOWED_FUNCTIONS', 
	'isset,empty,is_null,is_numeric,is_bool,is_integer,is_double,is_array,sizeof,is_finite,is_float,is_infinite,'.
	'is_int,is_long,is_nan,is_real,is_scalar,is_string,mt_rand,mt_srand,mt_getrandmax,rand,urlencode,urldecode,'.
	'uniqid,date,time,intval,htmlspecialchars,htmlspecialchars_decode,strtr,strpos,str_replace,str_ireplace,substr,'.
	'sprintf,abs,acos,acosh,asin,atan2,atanh,base_convert,bindec,ceil,cos,cosh,decbin,dechex,decoct,deg2rad,exp,expm1,'.
	'floor,fmod,getrandmax,hexdec,hypot,lcg_value,log10,log1p,log,max,min,octdec,pi,pow,rad2deg,rand,round,sin,sinh,'.
	'sqrt,srand,tan,tanh,cal_days_in_month,cal_from_jd,cal_info,cal_to_jd,easter_date,easter_days,frenchtojd,gregoriantojd,'.
	'jddayofweek,jdmonthname,jdtofrench,jdtogregorian,jdtojewish,jdtojulian,jdtounix,jewishtojd,jewishtojd,unixtojd,checkdate,'.
	'date_default_timezone_get,strtotime,date_sunset,gmdate,gmmktime,gmstrftime,idate,microtime,mktime,strftime,strptime,'.
	'strtotime,timezone_name_from_abbr,timezone_version_get,bcadd,bccomp,bcdiv,bcmod,bcmul,bcpow,bcpowmod,bcscale,bcsqrt,'.
	'bcsub,addcslashes,addslashes,bin2hex,chop,chr,chunk_split,convert_cyr_string,convert_uudecode,convert_uuencode,count,'.
	'count_chars,crc32,crypt,hebrev,hebrevc,hex2bin,html_entity_decode,htmlentities,htmlspecialchars_decode,htmlspecialchars,'.
	'lcfirst,levenshtein,ltrim,md5,metaphone,money_format,nl_langinfo,nl2br,number_format,ord,quoted_printable_decode,'.
	'quoted_printable_encode,quotemeta,rtrim,sha1,similar_text,soundex,sprintf,str_pad,str_repeat,str_rot13,str_shuffle,'.
	'strcasecmp,strchr,strcmp,strcoll,strcspn,strip_tags,stripcslashes,stripos,stripslashes,stristr,strlen,strnatcasecmp,'.
	'strnatcmp,strncasecmp,strncmp,strpbrk,strrchr,strrev,strripos,strrpos,strspn,strtolower,strtoupper,strtr,substr_compare,'.
	'substr_count,substr_replace,trim,ucfirst,ucwords,wordwrap,floatval,strval,implode,explode,array_keys,get_object_vars,is_object');

// Valid PHP tokens in expressions
define('DIV_PHP_VALID_TOKENS_FOR_EXPRESSIONS', 'T_ARRAY,T_ARRAY_CAST,T_BOOLEAN_AND,T_BOOLEAN_OR,T_BOOL_CAST,T_CHARACTER,T_CONSTANT_ENCAPSED_STRING,'.
	'T_DNUMBER,T_DOUBLE_CAST,T_EMPTY,T_INT_CAST,T_ISSET,T_IS_EQUAL,T_IS_GREATER_OR_EQUAL,T_SR,T_IS_IDENTICAL,T_IS_NOT_EQUAL,T_IS_NOT_IDENTICAL,'.
	'T_IS_SMALLER_OR_EQUAL,T_LNUMBER,T_LOGICAL_AND,T_LOGICAL_OR,T_LOGICAL_XOR,T_SL,T_SL_EQUAL,T_SR_EQUAL,T_STRING_CAST,T_STRING_VARNAME,'.
	'T_VARIABLE,T_WHITESPACE,T_CURLY_OPEN,T_INC,T_COMMENT,T_DOUBLE_ARROW,T_ENCAPSED_AND_WHITESPACE');

// Valid PHP tokens in macros
define('DIV_PHP_VALID_TOKENS_FOR_MACROS', 'T_AS,T_DO,T_DOUBLE_COLON,T_ECHO,T_ELSE,T_ELSEIF,T_FOR,T_FOREACH,T_IF,T_MOD_EQUAL,T_MUL_EQUAL,'.
	'T_OBJECT_OPERATOR,T_NUM_STRING,T_OR_EQUAL,T_PAAMAYIM_NEKUDOTAYIM,T_PLUS_EQUAL,T_PRINT,T_START_HEREDOC,T_SWITCH,'.
...
define('DIV_PHP_ALLOWED_METHODS', 'getRanges,asThis,atLeastOneString,getLastKeyOfArray,'.
	'getCountOfParagraphs,getCountOfSentences,getCountOfWords,htmlToText,'.
	'isArrayOfArray,isArrayOfObjects,isCli,isNumericList,jsonDecode,jsonEncode,isString,mixedBool,div');
...
class div {
		...
		if (strpos($this->__src, "$key.") !== false) {
			if (is_object($value)) {
				...
				foreach ( $vars as $kk => $v )
					$this->parseSubmatch($items, "$key.$kk", $v, $modifiers);
			}
			if (is_array($value))
			...
			if ($superkey . $key !== '')
				if (strpos($src, $superkey . $key) !== false) {
					...
					if (! is_scalar($itemsx[$key]))
						$vars = array_merge($vars, $this->getActiveVars($itemsx[$key], $superkey . $key . '.', $src));
				}
		}
	...
	final private function memory(&$items){
		...
		foreach ( $vars as $var )
			if (! isset($items[$var]) || strpos($var, '.') !== false)
			...
			switch ($o) {
				...
				case 'dateformat' :
					...
					while ( true ) {
						...
						if ($po !== false)
							$subsrc = substr($subsrc, 0, $po) . DIV_TAG_DATE_FORMAT_SEPARATOR . substr($subsrc, $po + $lsep);
				...
				case 'numberformat' :
					...
					while ( true ) {
						...
						if ($po !== false)
							$subsrc = substr($subsrc, 0, $po) . DIV_TAG_NUMBER_FORMAT_SEPARATOR . substr($subsrc, $po + $lsep);
				...
				case 'loops' :
					...
					while ( true ) {
						...
						$r = $this->getBlockRanges($src, $DIV_TAG_LOOP_BEGIN_PREFIX, $DIV_TAG_LOOP_BEGIN_SUFFIX, $DIV_TAG_LOOP_END_PREFIX, $DIV_TAG_LOOP_END_SUFFIX, $p, null, true);
				...
				case 'conditionals' :
					...
					while ( true ) {
						...
						$r = $this->getBlockRanges($src, $DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX, $DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX, $DIV_TAG_CONDITIONAL_TRUE_END_PREFIX, $DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX, $p, null, true);
						...
						$src = substr($src, 0, $ini) . DIV_TAG_CONDITIONAL_TRUE_BEGIN_PREFIX . $key . DIV_TAG_CONDITIONAL_TRUE_BEGIN_SUFFIX . $subsrc . DIV_TAG_CONDITIONAL_TRUE_END_PREFIX . $key . DIV_TAG_CONDITIONAL_TRUE_END_SUFFIX . substr($src, $end + $lsuffix);
					...
					while ( true ) {
						...
						$r = $this->getBlockRanges($src, $DIV_TAG_CONDITIONAL_FALSE_BEGIN_PREFIX, $DIV_TAG_CONDITIONAL_FALSE_BEGIN_SUFFIX, $DIV_TAG_CONDITIONAL_FALSE_END_PREFIX, $DIV_TAG_CONDITIONAL_FALSE_END_SUFFIX, $p, null, true);
				...
				case 'tplvars' :
					...
					while ( true ) {
						...
						$src = substr($src, 0, $ini) . DIV_TAG_TPLVAR_BEGIN . str_replace($DIV_TAG_TPLVAR_PROTECTOR, DIV_TAG_TPLVAR_PROTECTOR, substr($src, $ini + $lbegin, $p1 - ($ini + $lbegin))) . DIV_TAG_TPLVAR_ASSIGN_OPERATOR . substr($src, $p1 + $loperator, $end - ($p1 + $loperator)) . DIV_TAG_TPLVAR_END . substr($src, $end + $lend);
				...
				case 'capsules' :
					$src = $this->translateKeyBlocks($src, $DIV_TAG_CAPSULE_BEGIN_PREFIX, $DIV_TAG_CAPSULE_BEGIN_SUFFIX, $DIV_TAG_CAPSULE_END_PREFIX, $DIV_TAG_CAPSULE_END_SUFFIX, DIV_TAG_CAPSULE_BEGIN_PREFIX, DIV_TAG_CAPSULE_BEGIN_SUFFIX, DIV_TAG_CAPSULE_END_PREFIX, DIV_TAG_CAPSULE_END_SUFFIX);
					break;
	 ...
	 * @return boolean
	 */
	final static function isValidPHPCode(
		$code, 
		$multi_lines = true, 
		$valid_tokens = null, 
		$invalid_vars = array('$this', '$_SESSION','$GLOBALS','$_POST','$_GET','$_FILES','$_COOKIE','$_ENV', '$_REQUEST'), 
		$allow_classes = false,
			...
			if (is_array($token)) {
				...
				switch ($n) {
					...
					case 'T_STRING' :
						...
						if ($last_token == 'T_CLASS') $classname = true;
						if ($last_token == 'T_FUNCTION') $funcname = true;
